...
export function parsirajZapisnik(html, { zapisnikId = null, url = null } = {}) {
  ...
  if (skupine.length < 4) return null
  ...
  return {
    ...
    rezultat,
    polcas: null,
    // Ikona pove le, da je gol padel; avtogola in enajstmetrovke stran ne
    // loči, zato ju ne izmišljamo — oboje ostane navaden gol.
    goli: poVrsti('gol').map((g) => ({ ...g, avtogol: false, enajstmetrovka: false })),
    zgresene: [],
    rumeni: poVrsti('rumeni'),
    rdeci: poVrsti('rdeci'),
    menjave: ekipe.flatMap((e, idx) => menjaveEkipe(e.postava, e.rezerve, idx)),
    opozorila: [],
  }
}
